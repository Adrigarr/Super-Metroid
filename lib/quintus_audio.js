...
Quintus.Audio = function(Q) {
  ...
  ////....................
  // HACK PARA CHROME
  // Desactivamos WebAudio para poder hacer stop dos veces <https://github.com/cykod/Quintus/issues/91>
    ...
    Q.audio.play = function(s,options) {
      ...
      for (var i=0;i<Q.audio.channels.length;i++) {
        ...
        if (!Q.audio.channels[i]['loop'] && Q.audio.channels[i]['finished'] < now) {
          ...
          // If we're looping - just set loop to true to prevent this channel
          // from being used.
          if(options && options['loop']) {
            Q.audio.channels[i]['loop'] = true;
            Q.audio.channels[i]['channel'].loop = true;
          } else {
            /// FIXED: Si no ponemos esto y sobreescribimos el canal entonces se queda en loop
            Q.audio.channels[i]['channel'].loop = false;
